...
const deleteLogs = async (directory, daysToKeepLog) => {
    // check before execute, daysToKeep must keep at least 0 (today) 
    if (daysToKeepLog < 0)
        return;
    ...
    const begin = new Date(end.valueOf() - (daysToKeepLog * 24 * 60 * 60 * 1000));

    // loop for delete files
    logger.debug(`log older than ${format(begin, 'YYYY-MM-DD')} will be delete!`);
